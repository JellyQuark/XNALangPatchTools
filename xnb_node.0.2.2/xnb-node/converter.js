...
function ObjectToXnb(data) {
    ...
    if(data.xnbData.compressed) {
        let decompressedData = new Buffer(decompressedBuffer.length);
        decompressedBuffer.buffer.copy(decompressedData, 0, 0, decompressedBuffer.length);
        let compressedBuffer = new writer.BufferWriter(compress.compress(decompressedData));
        ...
        buffer.concat(decompressedBuffer);
    }
    
    let ret = new Buffer(buffer.length);
    buffer.buffer.copy(ret, 0, 0, buffer.length);
    return ret;
}
