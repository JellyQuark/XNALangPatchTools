...
class BufferWriter {
    constructor(buffer) {
        if(!buffer) {
            this.buffer = new Buffer(104857600);
            this.length = 0;
        } else {
            this.buffer = buffer;
    ...
    concat(bufferWriter) {
        bufferWriter.buffer.copy(this.buffer, this.length, 0, bufferWriter.length);
        this.length += bufferWriter.length;
        //this.buffer = Buffer.concat([this.buffer, buffer]);
        this.buffer.type = ref.types.byte;
    }

    writeAscii(text) {
        let buffer = new BufferWriter(Buffer(text, 'ascii'));
        this.concat(buffer);
    }

    writeUtf8(text) {
        let buffer = new BufferWriter(Buffer(text, 'utf8'));
        this.concat(buffer);
    }

    writeByte(number) {
        let buffer = new BufferWriter(Buffer(1));
        buffer.buffer.writeUInt8(number, 0);
        this.concat(buffer);
    }

    writeInt32LE(number) {
        let buffer = new BufferWriter(Buffer(4));
        buffer.buffer.writeInt32LE(number, 0);
        this.concat(buffer);
    }

    writeUInt32LE(number) {
        let buffer = new BufferWriter(Buffer(4));
        buffer.buffer.writeUInt32LE(number, 0);
        this.concat(buffer);
    }

    writeFloatLE(number) {
        let buffer = new BufferWriter(Buffer(4));
        buffer.buffer.writeFloatLE(number, 0);
        this.concat(buffer);
    }
...
class Texture2DWriter {
    write(buffer, imageData, writerResolver) {
        ...
        buffer.concat(new BufferWriter(data));
    }
}
...
class TBinWriter {
    write(buffer, tBinData, writerResolver) {
        ...
        buffer.concat(new BufferWriter(tBinData.data));
    }
}
...
class CharWriter {
    write(buffer, char, writerResolver) {
        ...
        buffer.concat(new BufferWriter(charBuffer.slice(0, size)));
    }
}
...
class StringWriter {
    write(buffer, text, writerResolver) {
        ...
        buffer.concat(new BufferWriter(stringBuffer.slice(0, size)));
    }
}
